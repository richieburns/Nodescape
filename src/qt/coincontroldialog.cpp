    ...
    platformStyle(platformStyle)
{
    ...
    ui->treeWidget->setColumnWidth(COLUMN_DATE, 105);
    ui->treeWidget->setColumnWidth(COLUMN_CONFIRMATIONS, 95);
    ...
    // Center align header for sort arrows
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = BitcoinUnits::NSX;
    ...
    BOOST_FOREACH(const PAIRTYPE(QString, std::vector<COutput>)& coins, mapCoins) {
        ...
        BOOST_FOREACH(const COutput& out, coins.second) {
            ...
            if(ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress))
            {
                ...
                // if listMode or change => show nodescape address. In tree mode, address is not shown again for direct wallet address outputs
