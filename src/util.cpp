...
const char * const BITCOIN_CONF_FILENAME = "nodescape.conf";
const char * const BITCOIN_PID_FILENAME = "nodescaped.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "nodescape" is a composite category enabling all Nodescape-related debug output
            if(ptrCategory->count(string("nodescape"))) {
...
#else
    const char* pszModule = "nodescape";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\NodescapeCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\NodescapeCore
    // Mac: ~/Library/Application Support/NodescapeCore
    // Unix: ~/.nodescapecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NodescapeCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/NodescapeCore";
#else
    // Unix
    return pathRet / ".nodescapecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty nodescape.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override nodescape.conf
